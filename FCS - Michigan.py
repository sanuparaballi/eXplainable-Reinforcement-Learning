...
# -*- coding: utf-8 -*-
"""
Created on Sat Mar  8 17:29:45 2025
...
# ------------------------------
# Fuzzy Set, Knowledge Base, and Fuzzy Rule Definitions (same as before)
# ------------------------------
class FuzzySet:
...
class FuzzyRule:
    ...
    def __str__(self):
        antecedents = [f"x{i} set#{self.indices[i]}({self.fuzzy_kb.fuzzy_sets[i][self.indices[i]]})" 
                       for i in range(self.num_inputs)]
        consequent = ", ".join(f"{w:.2f}" for w in self.weights.flatten())
        return "IF " + " AND ".join(antecedents) + f" THEN action=[{consequent}]"

# ------------------------------
# Michigan LCS: The population is a set of individual fuzzy rules.
# ------------------------------
class MichiganLCS:
    def __init__(self, num_rules, num_inputs, action_dim, num_sets=7):
        self.num_inputs = num_inputs
        self.action_dim = action_dim
    ...
    def evaluate(self, env, episodes=1, max_steps=300):
        total_reward = 0.0
    ...
    def crossover(self, other):
        child = MichiganLCS(len(self.rules), self.num_inputs, self.action_dim)
...
def test_final_sequence_agent(agent, episodes=10, max_steps=1000, defuzz_method='weighted_average'):
    ...
    return avg_reward
# ------------------------------
# Michigan LCS Evolutionary System
# ------------------------------
def michigan_evolution(num_generations=50, population_size=50, num_rules=100,
                       mutation_rate=0.1, mutation_scale=0.1):
    env = gym.make("LunarLanderContinuous-v2") #, render_mode='human')
    num_inputs = env.observation_space.shape[0]
    action_dim = env.action_space.shape[0]
    # In Michigan style, we maintain a single population of rules (one MichiganLCS instance)
    population = [MichiganLCS(num_rules, num_inputs, action_dim) for _ in range(population_size)]
    
    for gen in range(num_generations):
        # Evaluate each MichiganLCS (aggregate performance over all rules)
        fitnesses = []
        for individual in population:
            fitness = individual.evaluate(env)
    ...
    env.close()
    best = population[0]
    print("Best MichiganFCS:")
    print(best)
    return best

if __name__ == "__main__":
    print("=== Version 4: Michigan Style LCS ===")
    best_michigan = michigan_evolution()
    
    
    test_final_sequence_agent(best_michigan, episodes=10, max_steps=300, defuzz_method='weighted_average')
